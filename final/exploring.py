...
def find_closest_point(possible_points, robot_loc, map_data):
    ...
    rospy.logerr(f"Robot area is {(0.22 / map_data.resolution, 0.19 / map_data.resolution)}")
    rospy.logerr(f"Distances are: {distances}")
    rospy.logerr(f"Points are: {points}")
    rospy.logerr(f"Robot location is: {robot_loc}")
