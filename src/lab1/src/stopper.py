...
# YOUR CODE HERE
	...
	relevant_idxs = np.where(ranges * np.abs(np.sin(thetas)) <= w/2)[0]
	shortest = np.min(ranges[relevant_idxs]) if len(relevant_idxs) > 0 else np.max(ranges)
	if shortest > 1:
		t.linear.x = shortest/2
